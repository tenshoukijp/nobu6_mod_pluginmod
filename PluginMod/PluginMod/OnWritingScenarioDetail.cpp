...
extern int getCurScenarioLocation();

// 関数とchar **のセットを覚えておく
map<FARPROC, char **> psnGetProcAdressAndPointerMap;

void ReadFromTaretDll(int nSubNo) {
		...
		if ( 0 <= nSN && nSN < SCENARIODATASTRUCT_NUM ) {
			...
			if ( psn ) {
				...
				if ( strlen(psn[nSubNo]) > 2 ) {
					// 書き換え
					WriteProcessMemory( GetCurrentProcess(), (LPVOID)(0x4D97C0), psn[nSubNo], strlen(psn[nSubNo])+1, NULL);
...
void WriteAsmJumperOnTenshouExeWritingScenarioDetail1st() {
	...
	WriteProcessMemory( GetCurrentProcess(), (LPVOID)(pTenshouExeJumpFromToOnTenshouExeWritingScenarioDetail1st), cmdOnTenshouExeJumpFromWritingScenarioDetail1st, 5, NULL); //5バイトのみ書き込む
}
...
void WriteAsmJumperOnTenshouExeWritingScenarioDetail2nd() {
	...
	// 構築したニーモニック命令をTENSHOU.EXEのメモリに書き換える
	WriteProcessMemory( GetCurrentProcess(), (LPVOID)(pTenshouExeJumpFromToOnTenshouExeWritingScenarioDetail2nd), cmdOnTenshouExeJumpFromWritingScenarioDetail2nd, 5, NULL); //5バイトのみ書き込む
}
...
void WriteAsmJumperOnTenshouExeWritingScenarioDetail3rd() {
	...
	// 構築したニーモニック命令をTENSHOU.EXEのメモリに書き換える
	WriteProcessMemory( GetCurrentProcess(), (LPVOID)(pTenshouExeJumpFromToOnTenshouExeWritingScenarioDetail3rd), cmdOnTenshouExeJumpFromWritingScenarioDetail3rd, 5, NULL); //5バイトのみ書き込む
}
